...
async def test_fetch_reports_exception_written_into_the_body(exception_tag_server, ch_client: AsyncChClient):
    with pytest.raises(ChClientError, match="FUNCTION_THROW_IF_VALUE_IS_NON_ZERO"):
        await ch_client.fetch(_LATE_FAILURE)


async def test_fetch_rows_reports_exception_written_into_the_body(exception_tag_server, ch_client: AsyncChClient):
    with pytest.raises(ChClientError, match="FUNCTION_THROW_IF_VALUE_IS_NON_ZERO"):
        await ch_client.fetch_rows(_LATE_FAILURE)


async def test_stream_reports_exception_written_into_the_body(exception_tag_server, ch_client: AsyncChClient):
...
async def test_stream_format_reports_exception_written_into_the_body(exception_tag_server, ch_client: AsyncChClient):
    with pytest.raises(ChClientError, match="FUNCTION_THROW_IF_VALUE_IS_NON_ZERO"):
        async for _ in ch_client.stream_format(_LATE_FAILURE, "Parquet"):
            pass


async def test_execute_reports_exception_written_into_the_body(exception_tag_server, ch_client: AsyncChClient):
    with pytest.raises(ChClientError, match="FUNCTION_THROW_IF_VALUE_IS_NON_ZERO"):
        await ch_client.execute(_LATE_FAILURE)


async def test_late_failure_reports_the_error_not_the_payload(exception_tag_server, ch_client: AsyncChClient):
...
async def test_late_failure_always_raises(ch_client: AsyncChClient):
    """Older servers cut the response short instead of tagging it; a truncated result must still raise."""
